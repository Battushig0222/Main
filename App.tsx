...
const MangaEditModal: React.FC<{
  ...
  return (
    <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 bg-black/90 backdrop-blur-xl">
      ...
      <div className="bg-[#0f0f0f] w-full max-w-2xl p-8 md:p-12 rounded-[2.5rem] border border-white/10 shadow-2xl space-y-8 max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            <div className="flex flex-col gap-4">
              <div className="relative w-full max-h-[200px] overflow-hidden rounded-2xl border border-white/5 bg-black">
                <img src={coverUrl} className="w-full h-full object-contain opacity-60" />
                <div className="absolute inset-0 flex items-center justify-center">
                  <button onClick={() => setIsEditingCover(true)} className="bg-indigo-600 px-6 py-2.5 rounded-xl font-black text-[10px] uppercase tracking-widest">Зураг засах</button>
                </div>
              </div>
        ...
        <div className="flex flex-col gap-3 pt-6 border-t border-white/5">
          <button onClick={() => onSave({ ...manga, title, author, description, coverUrl, status })} className="w-full bg-indigo-600 py-4 rounded-2xl font-black uppercase tracking-widest">Хадгалах</button>
          <button onClick={() => window.confirm('Устгах уу?') && onDelete()} className="w-full bg-red-600/10 text-red-500 py-4 rounded-2xl font-black uppercase tracking-widest border border-red-600/20">Устгах</button>
        </div>
      </div>
    </div>
  );
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-10">
          <div className="space-y-6">
            <div className="bg-zinc-900/50 p-6 rounded-[2rem] border border-white/5 space-y-5">
              <input type="number" step="0.1" value={num} onChange={e => setNum(e.target.value)} className="w-full bg-black border border-white/5 rounded-xl p-3 text-white font-bold" placeholder="1.0" />
              <input value={title} onChange={e => setTitle(e.target.value)} className="w-full bg-black border border-white/5 rounded-xl p-3 text-white font-bold" placeholder="Гарчиг" />
            </div>
            <div className="space-y-3">
              ...
              <label htmlFor="ch-pages-edit" className="w-full flex items-center justify-center gap-2 bg-indigo-600/10 text-indigo-400 p-4 rounded-2xl cursor-pointer hover:bg-indigo-600/20 font-black text-xs uppercase tracking-widest">Хуудас нэмэх</label>
              <button onClick={() => onSave({ ...chapter, number: parseFloat(num), title, pages })} className="w-full bg-indigo-600 font-black py-5 rounded-2xl shadow-xl shadow-indigo-600/30">Хадгалах</button>
            </div>
          </div>
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 md:p-8">
      <div className="mb-16 space-y-6 pt-10">
        <h1 className="text-5xl md:text-7xl font-black tracking-tighter text-white">DISCOVER <br/> <span className="text-indigo-600">NEW STORIES</span></h1>
        <div className="relative max-w-2xl">
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Хайх (Нэр эсвэл Зохиолч)..." className="w-full bg-[#0f0f0f] border border-white/5 rounded-[1.5rem] p-5 pl-14 text-white font-bold outline-none focus:border-indigo-600 shadow-xl" />
...
const MangaDetail: React.FC<{ 
  ...
  if (!manga) return <div className="p-40 text-center font-black text-4xl opacity-20 uppercase">Manga Not Found</div>;
      ...
      <div className="flex flex-col lg:flex-row gap-16 lg:gap-24">
        <div className="w-full lg:w-[400px] shrink-0 space-y-10">
          ...
          <div className="bg-[#0f0f0f] p-8 md:p-10 rounded-[2.5rem] border border-white/5 space-y-6">
            <div className="flex justify-between font-bold"><span className="text-zinc-600 uppercase text-[10px] tracking-widest">Author</span><span className="text-sm text-white">{manga.author}</span></div>
            <div className="flex justify-between font-bold"><span className="text-zinc-600 uppercase text-[10px] tracking-widest">Status</span><span className="text-sm text-indigo-400">{manga.status}</span></div>
            <div className="flex justify-between font-bold"><span className="text-zinc-600 uppercase text-[10px] tracking-widest">Rating</span><span className="text-sm text-white">⭐ {manga.rating}</span></div>
          </div>
        </div>
        <div className="flex-1 space-y-14">
          <div className="space-y-8">
            <div className="flex justify-between items-start flex-wrap gap-6">
              ...
              {isAdmin && <button onClick={() => setShowAddChapter(!showAddChapter)} className="bg-indigo-600 px-8 py-4 rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl"> {showAddChapter ? 'Болих' : 'Add Chapter'} </button>}
            </div>
            <p className="text-zinc-400 text-lg md:text-xl leading-relaxed max-w-3xl font-medium">{manga.description}</p>
          </div>

          {showAddChapter && (
            <form onSubmit={e => { e.preventDefault(); onUpdateManga({ ...manga, chapters: [...manga.chapters, { id: `ch-${Date.now()}`, number: parseFloat(chNumber), title: chTitle, pages: chPages, createdAt: new Date().toLocaleDateString() }] }); setShowAddChapter(false); setChPages([]); setChNumber(''); setChTitle(''); }} className="p-10 bg-[#0f0f0f] rounded-[3rem] space-y-8 border border-white/5 animate-slide-up shadow-2xl">
              <h3 className="text-xl font-black italic">NEW <span className="text-indigo-500">CHAPTER</span></h3>
              <div className="grid md:grid-cols-2 gap-6">
                <input type="number" step="0.1" value={chNumber} onChange={e => setChNumber(e.target.value)} className="w-full bg-black border border-white/5 rounded-2xl p-4 text-white font-bold outline-none" placeholder="Бүлэг #" required />
                <input value={chTitle} onChange={e => setChTitle(e.target.value)} className="w-full bg-black border border-white/5 rounded-2xl p-4 text-white font-bold outline-none" placeholder="Гарчиг" required />
              </div>
              ...
              <label htmlFor="bulk-pages" className="block border-2 border-dashed border-white/5 p-16 rounded-[2rem] text-center cursor-pointer text-zinc-500 font-black uppercase text-xs"> {chPages.length > 0 ? `${chPages.length} PAGES SELECTED` : 'UPLOAD PAGES'} </label>
              <button className="w-full bg-indigo-600 py-5 rounded-2xl font-black uppercase tracking-widest">Create Chapter</button>
            </form>
          )}
          ...
          <div className="space-y-8">
            ...
            <div className="grid gap-4">
              {[...manga.chapters].sort((a,b) => b.number - a.number).map(chapter => (
                <div key={chapter.id} className="bg-[#0f0f0f] hover:bg-zinc-900 p-6 md:p-8 rounded-[2.5rem] flex items-center justify-between group transition-all border border-white/5">
                  ...
                  {isAdmin && (
                    <div className="flex gap-3 opacity-0 group-hover:opacity-100 transition-all">
                      <button onClick={() => setEditingChapter(chapter)} className="p-4 bg-indigo-600/10 text-indigo-500 rounded-2xl font-black text-[10px] uppercase">Edit</button>
                      <button onClick={() => window.confirm('Устгах уу?') && onUpdateManga({ ...manga, chapters: manga.chapters.filter(c => c.id !== chapter.id) })} className="p-4 bg-red-600/10 text-red-500 rounded-2xl font-black text-[10px] uppercase">Del</button>
                    </div>
                  )}
                </div>
              ))}
    ...
    <div className="bg-[#050505] min-h-screen">
      <div className="max-w-4xl mx-auto pb-32">
        <div className="sticky top-0 bg-[#050505]/90 backdrop-blur-2xl p-6 flex items-center justify-between z-50 border-b border-white/5">
          <button onClick={() => window.history.back()} className="p-4 bg-white/5 rounded-2xl hover:bg-white/10 text-white">←</button>
          <div className="text-center">
            <h2 className="font-black text-lg text-white truncate max-w-[200px]">{manga?.title}</h2>
            <p className="text-[10px] text-indigo-500 font-black uppercase tracking-widest">Chapter {chapter.number}</p>
          </div>
...
const AdminPanel: React.FC<{ 
  ...
  const handleMangaSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const manga: Manga = { ...newManga, id: `m-${Date.now()}`, gallery: [], genre: ['Manga'], status: 'Ongoing', rating: 5.0, chapters: [], coverUrl: newManga.coverUrl || 'https://picsum.photos/400/600' };
    onAddManga(manga); navigate(`/manga/${manga.id}`);
  };
    ...
    <div className="max-w-7xl mx-auto p-4 md:p-16 space-y-16">
      <div className="bg-[#0f0f0f] p-10 md:p-16 rounded-[3.5rem] border border-white/5 shadow-2xl space-y-12 relative overflow-hidden">
        ...
        <div className="flex items-center justify-between flex-wrap gap-8">
          <div><h1 className="text-5xl font-black text-white italic tracking-tighter">ADMIN <span className="text-indigo-500">DASHBOARD</span></h1><p className="text-zinc-500 font-bold text-sm uppercase tracking-widest mt-2">Manage your collection and sync to cloud.</p></div>
          <div className="flex flex-col gap-2">
            <div className={`px-6 py-2 rounded-full text-[10px] font-black uppercase tracking-widest border ${cloudStatus.includes('Амжилттай') ? 'bg-indigo-600/10 text-indigo-400 border-indigo-600/20' : 'bg-zinc-900 text-zinc-500 border-white/5'}`}>Status: {cloudStatus}</div>
            <div className="flex gap-2">
              <button onClick={onSyncToCloud} className="bg-indigo-600 px-6 py-2 rounded-xl text-[10px] font-black uppercase">Sync Up</button>
              <button onClick={onFetchFromCloud} className="bg-zinc-800 px-6 py-2 rounded-xl text-[10px] font-black uppercase">Sync Down</button>
            </div>
          </div>
        </div>
        <div className="flex gap-8 border-t border-white/5 pt-8">
          <button onClick={() => setActiveTab('manga')} className={`text-sm font-black uppercase tracking-[0.2em] pb-2 border-b-2 transition-all ${activeTab === 'manga' ? 'border-indigo-600 text-white' : 'border-transparent text-zinc-600'}`}>Manga Content</button>
          <button onClick={() => setActiveTab('admins')} className={`text-sm font-black uppercase tracking-[0.2em] pb-2 border-b-2 transition-all ${activeTab === 'admins' ? 'border-indigo-600 text-white' : 'border-transparent text-zinc-600'}`}>Admin Managers</button>
        </div>
      </div>
      ...
      {activeTab === 'manga' ? (
        <div className="grid lg:grid-cols-2 gap-16">
          <form onSubmit={handleMangaSubmit} className="space-y-10 bg-[#0f0f0f] p-10 md:p-14 rounded-[3.5rem] border border-white/5 shadow-2xl">
            ...
            <div className="space-y-6">
              ...
              <textarea value={newManga.description} onChange={e => setNewManga({...newManga, description: e.target.value})} className="w-full bg-black border border-white/5 rounded-2xl p-5 text-white h-44 font-medium" placeholder="Description" required />
              ...
              <label htmlFor="add-m-c" className="block border-2 border-dashed border-white/5 p-10 rounded-[2rem] text-center cursor-pointer text-zinc-600 font-black uppercase text-xs"> {newManga.coverUrl ? 'COVER READY' : 'SELECT COVER IMAGE'} </label>
              <button className="w-full bg-indigo-600 py-6 rounded-2xl font-black uppercase tracking-widest shadow-2xl">Register Manga</button>
            </div>
          </form>
          <div className="bg-[#0f0f0f] p-10 md:p-14 rounded-[3.5rem] border border-white/5 space-y-10">
            <h2 className="text-3xl font-black italic">MANAGE <span className="text-indigo-500">LIST</span></h2>
            <div className="space-y-4 max-h-[600px] overflow-y-auto pr-4 scrollbar-hide">
              {mangaList.map(m => (
                <div key={m.id} className="flex items-center justify-between p-5 bg-black rounded-[2rem] border border-white/5 group hover:border-indigo-600/40">
                  <div className="flex items-center gap-6"><img src={m.coverUrl} className="w-16 h-20 object-cover rounded-xl" /> <div className="font-black text-white">{m.title}</div> </div>
                  <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-all">
                    ...
                    <button onClick={() => onDeleteManga(m.id)} className="p-3 bg-red-600/20 text-red-500 rounded-xl text-[10px] font-black uppercase">Del</button>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="grid lg:grid-cols-2 gap-16">
          <form onSubmit={e => { e.preventDefault(); onAddAdmin(newAdmin); setNewAdmin({username:'', password:''}); }} className="space-y-10 bg-[#0f0f0f] p-10 md:p-14 rounded-[3.5rem] border border-white/5 shadow-2xl">
            <h2 className="text-3xl font-black italic">NEW <span className="text-indigo-500">ADMIN</span></h2>
            <div className="space-y-6">
              <input value={newAdmin.username} onChange={e => setNewAdmin({...newAdmin, username: e.target.value})} className="w-full bg-black border border-white/5 rounded-2xl p-5 text-white font-bold" placeholder="Username" required />
              <input type="password" value={newAdmin.password} onChange={e => setNewAdmin({...newAdmin, password: e.target.value})} className="w-full bg-black border border-white/5 rounded-2xl p-5 text-white font-bold" placeholder="Password" required />
              <button className="w-full bg-indigo-600 py-6 rounded-2xl font-black uppercase tracking-widest text-sm">Add New Administrator</button>
            </div>
          </form>
          <div className="bg-[#0f0f0f] p-10 md:p-14 rounded-[3.5rem] border border-white/5 space-y-10">
            <h2 className="text-3xl font-black italic">ACTIVE <span className="text-indigo-500">ADMINS</span></h2>
            <div className="space-y-4">
              {admins.map((a, i) => (
                <div key={i} className="flex items-center justify-between p-6 bg-black rounded-[2rem] border border-white/5 group">
                  <div className="flex items-center gap-4">
                    <div className="w-10 h-10 rounded-full bg-indigo-600/20 flex items-center justify-center font-black text-indigo-500">A</div>
                    <div><div className="font-black text-white">{a.username}</div><div className="text-[10px] text-zinc-600 font-bold uppercase">Admin Role</div></div>
                  </div>
                  {!a.isSuperAdmin && currentUser?.username === 'Battushig' && (
                    <button onClick={() => onDeleteAdmin(a.username)} className="text-red-500 hover:text-red-400 font-black text-[10px] uppercase tracking-widest opacity-0 group-hover:opacity-100 transition-all">Revoke</button>
                  )}
                  {a.isSuperAdmin && <span className="text-indigo-500 text-[9px] font-black uppercase tracking-widest">Super Admin</span>}
                </div>
              ))}
            </div>
          </div>
...
const App: React.FC = () => {
  ...
  const handleSyncToCloud = async () => {
    ...
    try {
      await sb.from('manga').upsert(mangaList.map(m => ({ id: m.id, data: m })));
      await sb.from('config').upsert({ id: 'admins_list', data: admins });
      setCloudStatus('Амжилттай хуулагдлаа');
    } catch (e: any) { setCloudStatus('Error: ' + e.message); }
  };
  ...
  const handleFetchFromCloud = async () => {
    ...
    try {
      ...
      if (mData) setMangaList(mData.map((i: any) => i.data));
      const { data: cData } = await sb.from('config').select('*').eq('id', 'admins_list').single();
      if (cData) setAdmins(cData.data);
      ...
      <div className="min-h-screen flex flex-col selection:bg-indigo-600 selection:text-white">
        ...
        <main className="flex-1 bg-[#050505]">
          <Routes>
            ...
            <Route path="/admin" element={authState.user?.role === 'admin' ? <AdminPanel mangaList={mangaList} admins={admins} onAddManga={m => setMangaList([m, ...mangaList])} onSyncToCloud={handleSyncToCloud} onFetchFromCloud={handleFetchFromCloud} onDeleteManga={id => setMangaList(mangaList.filter(m => m.id !== id))} onAddAdmin={a => setAdmins([...admins, a])} onDeleteAdmin={u => setAdmins(admins.filter(a => a.username !== u))} cloudStatus={cloudStatus} currentUser={authState.user} /> : <Home mangaList={mangaList} />} />
          </Routes>
        </main>
        {showAuthModal && (
          <div className="fixed inset-0 z-[500] flex items-center justify-center p-4 bg-black/95 backdrop-blur-3xl">
            <div className="bg-[#0f0f0f] w-full max-w-md p-12 rounded-[3.5rem] border border-white/5 shadow-2xl scale-in">
              ...
              <form onSubmit={handleLogin} className="space-y-8">
                ...
                <button className="w-full bg-indigo-600 py-5 rounded-2xl font-black uppercase tracking-widest shadow-2xl">Authorize</button>
              </form>
              <button onClick={() => setShowAuthModal(false)} className="mt-8 w-full text-[10px] text-zinc-600 font-black uppercase tracking-[0.3em]">Cancel</button>
            </div>
          </div>
        )}
